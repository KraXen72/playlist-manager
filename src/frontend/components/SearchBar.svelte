<script lang="ts">
	...
	let options = {
		search: (input: string) => {
			...
			res = res.filter((item) => {
				let inPlaylist = $currPlaylist.some(
					(song) => song.fullpath === item.fullpath
				);
				return !inPlaylist;
			});

			if ($playlistOnlyMode.real && searchMode.artist) {
				searchMode.artist = false;
			}
			...
			if (!searchMode.artist) {
				//normal
				res = res.filter((song) =>
					song.filename.toLowerCase().includes(input.toLowerCase())
				);
			} else if (searchMode.artist) {
				//artist
				res = res
					.filter((song) => song.type !== "playlist")
					.filter((song) =>
						$tagDB[song.filename].artist
							.toLowerCase()
			...
			if ($playlistOnlyMode.real) {
				...
				res = res.sort((a, b) => {
					...
					if (
						a.type === "playlist" &&
						b.type === "song" &&
						a.prettyName?.toLowerCase().startsWith(lowInput)
					) {
						return -1;
					} else if (
						a.type === "song" &&
						b.type === "playlist" &&
						b.prettyName?.toLowerCase().startsWith(lowInput)
					) {
						return 1;
					} else {
						return 0;
		...
		onUpdate: async (results: [], selectedIndex: number) => {
			...
			if (filename !== "nothing_selected") {
				if (selsong.type === "song") {
					...
					let isCom = Object.keys($config.comPlaylists).includes(
						selsong.fullpath
					);
					$detailsData = {
...
<div class="comp" class:notready={disabled}>
	...
	<Group variant="outlined">
		<Button
			variant="outlined"
			title="find songs with hard to type titles"
...
{#if disabled}
	<div class="comp blocker">
		<div class="progress-text">Fetching metadata for songs...</div>
		<div class="progress-bar">
			<LinearProgress
				progress={precisionRound(cacheTagsProgress / 100, 2)}
			/>
			{cacheTagsProgress}%
		</div>
	</div>
